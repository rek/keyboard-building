...
export function checkCompatibility(choices: UserChoices): CompatibilityWarning[] {
  ...
  if (choices.firmware === 'zmk' && choices.controller !== 'nice-nano') {
    warnings.push({
      severity: 'error',
      message: 'ZMK firmware requires nice!nano controller. QMK, Vial, and KMK are not supported with nice!nano.',
      affectedChoices: ['firmware', 'controller'],
    });
  }

  if ((choices.firmware === 'qmk' || choices.firmware === 'vial') && choices.controller === 'nice-nano') {
    warnings.push({
      severity: 'error',
  ...
  if (choices.firmware === 'kmk' && choices.controller !== 'rp2040') {
    warnings.push({
      severity: 'error',
      message: 'KMK firmware requires RP2040-based controllers (CircuitPython compatible). Consider using QMK/Vial for ATmega32U4 boards.',
      affectedChoices: ['firmware', 'controller'],
    });
  }
  ...
  if (choices.features.trackball) {
    if (choices.buildMethod !== 'custom-pcb') {
      warnings.push({
        severity: 'warning',
        message: 'Trackball integration is extremely complex with handwired builds. A custom PCB is highly recommended.',
    ...
    if (choices.controller === 'pro-micro') {
      warnings.push({
        severity: 'warning',
        message: 'Trackball requires 6 pins for SPI communication. Pro Micro may not have enough pins for a full matrix.',
  ...
  if (choices.layout.formFactor === 'ergonomic-3d' && choices.buildMethod === 'pcb-kit') {
    warnings.push({
      severity: 'info',
      message: 'Ergonomic 3D layouts typically require custom 3D printed cases. PCB kits may not include compatible cases.',
  ...
  if (choices.switchType === 'choc-v1' && choices.layout.formFactor === 'ergonomic-3d') {
    warnings.push({
      severity: 'info',
      message: 'Choc v1 switches work great with 3D printed cases but require careful case design for the lower profile.',
  ...
  if (choices.features.hotswap && choices.buildMethod === 'handwired') {
    warnings.push({
      severity: 'warning',
      message: 'Hot-swap sockets are difficult to integrate into handwired builds. Consider a PCB-based build instead.',
      affectedChoices: ['features', 'buildMethod'],
    });
  }
  ...
  if (choices.features.rgb && choices.controller === 'pro-micro') {
    ...
    if (keyCount > 40) {
      warnings.push({
        severity: 'warning',
        message: 'RGB LEDs for 40+ keys can be complex to wire and may cause power issues. Consider per-key vs underglow.',
        affectedChoices: ['features', 'controller'],
      });
    }
  }
...
export function getCompatibilityStatus(warnings: CompatibilityWarning[]): 'ok' | 'warnings' | 'errors' {
