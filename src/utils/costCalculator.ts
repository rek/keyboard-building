...
export function calculateCost(choices: UserChoices): CostEstimate {
  ...
  if (choices.controller) {
    const controllerPrice = costDatabase.controllers[choices.controller as keyof typeof costDatabase.controllers]?.base || 0;
  ...
  if (choices.switchType) {
    ...
    const switchPrice = costDatabase.switches[choices.switchType as keyof typeof costDatabase.switches]?.midrange || 0;
  ...
  if (choices.buildMethod === 'handwired') {
    breakdown.hardware = costDatabase.buildMethodExtras.handwired.wire +
    ...
    breakdown.hardware = costDatabase.buildMethodExtras['custom-pcb'].diodes +
  ...
  if (choices.connectivity === 'trrs') {
    breakdown.connectivity = costDatabase.connectivity['trrs-cable'] +
                            (costDatabase.connectivity['trrs-jack'] * 2);
  } else if (choices.connectivity === 'wireless') {
    breakdown.connectivity = (costDatabase.connectivity['battery-lipo'] * 2) +
                            (costDatabase.connectivity['power-switch'] * 2);
  }
